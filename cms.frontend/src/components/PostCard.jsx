...
const IMAGE_BASE_URL = "https://localhost:7238"; // Đường dẫn Backend của bạn

// Component con nhận dữ liệu 'post' từ component cha truyền xuống
function PostCard({ post }) {

    // Hàm bổ trợ: Chuẩn hóa đường dẫn ảnh an toàn, tránh lỗi thiếu/thừa dấu "/"
    const getImageUrl = (url) => {
        if (!url) return 'https://via.placeholder.com/400x250?text=No+Image'; // Ảnh mặc định nếu bài viết không có ảnh
        if (url.startsWith('http')) return url; // Nếu API trả về link tuyệt đối có sẵn http
    ...
    return (
        <div className="card h-100 shadow-sm border-0 blog-card-hover" style={{ borderRadius: '12px', overflow: 'hidden', transition: '0.3s' }}>
            ...
            <div className="blog-image-wrapper" style={{ height: '220px', overflow: 'hidden', backgroundColor: '#f8fafc' }}>
                <img
                    ...
                    onError={(e) => {
                        // Nếu đường dẫn ảnh bị lỗi 404 từ backend, nạp ảnh placeholder để giao diện đẹp mắt
                        e.target.src = 'https://via.placeholder.com/400x250?text=KieuCMS+Blog';
                    }}
                />
            </div>
